...
const StorageManagementTab: React.FC = () => {
  ...
  const clearLocalStorage = () => {
    if (window.confirm("LocalStorage의 모든 피어몰 데이터를 삭제하시겠습니까? 이 작업은 되돌릴 수 없으며, 브라우저 캐시/데이터 삭제 시에도 지워질 수 있습니다.")) {
      // 실제 구현에서는 localStorage.clear() 또는 특정 항목만 삭제
      setStorageData(prev => ({
        ...prev,
        localStorage: {
          ...prev.localStorage,
  ...
  const requestFileSystemPermission = async () => {
    // 참고: window.showDirectoryPicker는 모든 브라우저에서 지원하지 않습니다.
    // 이 API를 사용할 때는 기능 탐지 및 대체 로직이 필요합니다.
    try {
      // File System Access API가 지원되는 브라우저에서만 작동
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            <Button variant="outline" size="sm">데이터 보기</Button>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            <Button variant="outline" size="sm">데이터 관리</Button>
          </div>
          <div className="bg-yellow-50 p-2 text-xs text-yellow-700 rounded border-l-3 border-yellow-400">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="bg-yellow-50 p-2 text-xs text-yellow-700 rounded border-l-3 border-yellow-400">
            <Link className="inline mr-1 h-3 w-3" /> 별도 설정 및 외부 네트워크 연결이 필요합니다.
          </div>
        </CardContent>
      </Card>
    </div>
