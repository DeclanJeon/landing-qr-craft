...
const ShopFooter: React.FC<ShopFooterProps> = ({ shopName, shopUrl, shopData }) => {
  ...
  return (
    <footer className={`${footerSettings.background} ${footerSettings.textColor}`}>
      <div className="container mx-auto px-4 py-16">
        <div className="flex flex-col md:flex-row justify-between">
          <div className="mb-10 md:mb-0">
            <h3 className="text-2xl font-serif font-medium mb-6">{shopName}</h3>
            <p className={`${footerSettings.textColor}/80 max-w-md leading-relaxed text-base`}>
              {shopName}은 고객님께 최고의 제품과 서비스를 제공하기 위해 노력하고 있습니다.
              핵심 가치는 품질, 디자인, 그리고 고객 만족입니다.
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-8 sm:grid-cols-2">
            <div>
              ...
              <ul className="space-y-3">
                <li><Link to={`/shop/${actualShopUrl}/about`} className={`${footerSettings.textColor}/70 hover:${footerSettings.textColor} transition-colors`}>소개</Link></li>
            ...
            <div>
              <h4 className="text-lg font-medium mb-4 text-gray-200">연락처 정보</h4>
              <ul className={`space-y-3 ${footerSettings.textColor}/70`}>
                ...
                {(shopData?.contactNumber || footerSettings.contactNumber) && (
                  <li className="flex items-center">
                    <Link to={`/shop/${actualShopUrl}/call`} className={`flex items-center hover:${footerSettings.textColor} transition-colors`}>
        ...
        <div className={`border-t border-${footerSettings.textColor}/10 mt-10 pt-8 text-center ${footerSettings.textColor}/60`}>
          ...
          <p className="mt-4 text-sm">Powered by Peermall</p>
        </div>
      </div>
    </footer>
  );
